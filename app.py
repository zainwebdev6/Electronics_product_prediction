...
df = pd.read_csv('ElectronicsProducts.csv')
...
def tokenization(txt):
    ...
    stemming = [stmer.stem(w) for w in tokens if not w in stopwords.words('english')]
...
def recommation(query):
    ...
    final_df = df.sort_values(by=['similarity'],ascending=False).head(20)[['name','brand']]
    return final_df
